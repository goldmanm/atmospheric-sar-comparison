...
from mereau2000 import sar,get_n_H
...
""")])]], family='R_Addition_MultipleBond', template=['Cd_R', 'CsJ'])
    ...
    def test_get_n_H(self):
        n_H = get_n_H(self.reaction.products[0].molecule[0])
        self.assertEqual(n_H, 2)

    def test_get_arrhenius(self):
        "For ethoxyl radical decomposition"
        rate = self.sar.get_arrhenius(self.sar,self.reaction)
        self.assertAlmostEqual(rate.Ea.value,18.4,places=1)
        self.assertAlmostEqual(rate.A.value, self.A)
    ...
    def test_get_rate(self):
        ...
        self.assertAlmostEqual(rate,3.2,places=1) # rate from table 5
